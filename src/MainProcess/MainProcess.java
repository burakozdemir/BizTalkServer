    ...
    private static void work(Job job) throws IOException {
        ...
        for (String dest : destinations) {
            try {
                ...
                try {
                    System.out.println("Gir buraya");
                    LogClient.LogDesc("Machine with " + dest + " is not avaliable.", job.getOwner(), LogLevel.ERROR);
                    if (job.getOrchFlag() != 0) {
                        dbHandler.updateOrchestration(job.getOrchFlag(), "Status", StatusCodes.ERROR);
                        dbHandler.updateJobsSuccesfully(job.getOrchFlag(), StatusCodes.ERROR);
                    } else {
                        dbHandler.updateJob(job.getOwner(), "Status", StatusCodes.ERROR);
                    }
                    isValid = false;
                } catch (Exception e1) {
                    break;
        ...
        try {
            ...
            while (currentJob.getRuleId() != 0) {
                ...
                if (responseOfBRE == 'T') {
                    ...
                    dbHandler.updateJobsSuccesfully(orchestration.getId(), StatusCodes.ERROR);
            ...
            dbHandler.updateJobsSuccesfully(orchestration.getId(), StatusCodes.SUCCESS);
            ...
            LogClient.LogOrch(orchestrationCapsule, LogLevel.INFO);

            orchList.remove(orchestration);
            System.out.println("---- Orchestration has been completed! ----\n");
        } catch (Exception e) {
            e.printStackTrace();
